...
export default function TradingTerminalPage() {
  ...
  useEffect(() => {
    ...
    const connectWs = async () => {
      ...
      ws.onerror = (err) => {
        // Downgraded to warn: WS errors are transient network events (not fatal app errors)
        console.warn("Pump.fun WebSocket connection issue (will auto-reconnect):", err);
      };
        ...
        try {
          ...
          if (!botPosition) {
            ...
            if (bot.strategy === 'RSI Pullback' && closes.length >= 2) {
              ...
              // Volume confirmation: current volume exceeds the 5-period average
              ...
              // 2. Agent Momentum (volume & price action)
              ...
              if (targetCoinData) {
                ...
                // Advanced Anti-Rug & Social Check
                ...
                if (isScamSpam) {
                  addBotLogRef.current(bot.id, "Pump.fun Sniper", `Achat $${targetCoinData.symbol} ANNULÉ : Alerte Scam/Spam détectée dans les métadonnées.`, 'info');
                } else if (!isCreatorSafe) {
                  addBotLogRef.current(bot.id, "Pump.fun Sniper", `Achat $${targetCoinData.symbol} ANNULÉ : Auteur suspect ou contrat créateur invalide.`, 'info');
                } else {
                  if (mode === 'PRECOCE') {
                    // Ultra early sniper: Buy early curve launches having socials
                    if (curveProgress < 12) {
                      trigger = true;
                      details = `[Ultra-Précoce] Jeton repéré dès son lancement. Curve: ${curveProgress.toFixed(1)}%. Socials: ${hasSocials ? 'Actifs' : 'Non détectés'}.`;
                    }
                  } else if (mode === 'MOMENTUM') {
                    // Momentum: Social traction required (replies + socials links)
                    const momentumScore = (replies * 6) + (hasSocials ? 30 : 0);
                    if (momentumScore > 75) {
                      trigger = true;
                      details = `[Momentum] Traction et confiance sociale élevées. Réponses: ${replies}, Socials: OUI (Score: ${momentumScore.toFixed(0)}%).`;
                    } else {
                      addBotLogRef.current(bot.id, "Pump.fun Sniper", `Jeton $${targetCoinData.symbol} écarté (Momentum insuffisant: Score ${momentumScore.toFixed(0)}% < 75%).`, 'info');
                    }
                  } else if (mode === 'RAYDIUM') {
                    // RaydiumCompletion logic
                    if (curveProgress >= 78 && hasSocials) {
                      trigger = true;
                      details = `[Raydium Proche] Bonding Curve complétée à ${curveProgress.toFixed(1)}%. Migration Raydium imminente avec réseaux validés.`;
                    }
                  }
                }
            ...
            if (signal) {
              ...
              if (isBotReal && bot.strategy === 'Pump.fun Sniper Bot' && targetCoinData) {
                ...
                executeRealPumpTrade({
                  ...
                  if (res.success && res.txHash) {
                    addBotLogRef.current(bot.id, bot.strategy, `[ACHAT RÉEL RÉUSSI] Transaction confirmée sur Solana ! Hash: ${res.txHash.slice(0, 16)}...`, 'trade');
                    ...
                    addBotLogRef.current(bot.id, bot.strategy, `[ÉCHEC ACHAT RÉEL] ${res.error || 'Erreur réseau/RPC Solana.'}`, 'error');
                  }
                });
              } else {
  ...
  const closePositionById = (posId: string, exitPrice: number, reason: string) => {
    ...
    if (p.botId) {
      ...
      // Compute the new bots array synchronously from the ref (avoids calling setState inside a setState reducer)
      ...
      const updatedBots = botsRef.current.map(b => {
        if (b.id === botIdVal) {
          ...
          if (nextCapital <= 0) {
            nextStatus = 'STOPPED';
            circuitBreakerLogs.push({ id: b.id, strategy: b.strategy, message: `[CIRCUIT BREAKER] Capital du bot épuisé (0 $). Arrêt automatique.` });
          } else if (nextNetProfit <= maxDrawdownLimit) {
            nextStatus = 'STOPPED';
            circuitBreakerLogs.push({ id: b.id, strategy: b.strategy, message: `[CIRCUIT BREAKER] Perte cumulative de ${Math.abs(nextNetProfit).toFixed(2)} $ (limite de -15% atteinte). Arrêt automatique.` });
          } else if (nextLosses >= 3) {
            nextStatus = 'STOPPED';
            circuitBreakerLogs.push({ id: b.id, strategy: b.strategy, message: `[CIRCUIT BREAKER] 3 pertes consécutives subies. Arrêt automatique du bot pour protéger le capital.` });
          }
      ...
      // Log circuit breaker events AFTER setState, never inside the updater
  ...
  // Keep refs pointing to the latest versions so intervals don't hold stale closures
  ...
  const handleToggleBot = (botId: string) => {
    ...
    // Log BEFORE setBots to avoid calling setState inside a setState reducer
