    ...
    <div className="space-y-6">
      ...
      <Card className="bg-[#14101a] border-white/10 rounded-2xl p-5 space-y-5 shadow-xl">
              ...
              <SelectContent className="bg-[#14101a] border-white/10 text-white">
                {tradingMode === 'DEMO' ? (
                  <>
                    ...
                    <SelectItem value="SuperTrend Momentum" className="focus:bg-white/10 focus:text-white cursor-pointer">
                      ⚡ SuperTrend Volatility Momentum
                    </SelectItem>
                    <SelectItem value="VWAP Breakout" className="focus:bg-white/10 focus:text-white cursor-pointer">
        ...
        <form onSubmit={handleStartBot} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-white/40 uppercase font-headline">Recherche Multi-Actifs / Sélection Actif</label>
            {botStrategy === 'AI Autopilot (Machine à Cash)' ? (
              ...
              <Select value={botPair} onValueChange={setBotPair}>
                ...
                <SelectContent className="bg-[#14101a] border-white/10 text-white">
                  {currencyPairs.map((cp) => (
                    <SelectItem key={cp.value} value={cp.value} className="focus:bg-white/10 focus:text-white cursor-pointer">
          ...
          {botStrategy !== 'AI Autopilot (Machine à Cash)' && botStrategy !== 'Pump.fun Sniper Bot' && (
            <>
              {/* Timeframe for Technical Strategies */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-white/40 uppercase font-headline">Unité de Temps de Recherche (Timeframe)</label>
                <Select value={botTimeframe} onValueChange={setBotTimeframe}>
                  <SelectTrigger className="w-full h-11 bg-white/5 border border-white/10 rounded-xl px-3 text-sm text-white font-body focus:ring-[#c2ff0c]">
                    <SelectValue placeholder="Choisir une unité" />
                  </SelectTrigger>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[10px] text-emerald-400 font-mono font-semibold">
                Disponible: {tradingMode === 'REAL' 
                  ? `${solanaBalance?.toFixed(3)} SOL (${formatSolToUsdAndHtg(solanaBalance).combinedLabel})` 
                  : `${balance.toFixed(0)} $ (≈ ${formatUsdToHtg(balance)})`}
              </span>
            </div>
            <Input
              type="number"
      ...
      <Card className="bg-[#14101a] border-white/10 rounded-2xl p-5 shadow-xl">
        <CardHeader className="p-0 mb-4">
          <CardTitle className="text-sm font-bold uppercase tracking-wider text-white/70 font-headline flex items-center gap-2">
            ...
            {tradingMode === 'DEMO' ? "Mes Bots Démo" : "Mes Bots Réels"} ({filteredBots.length})
          </CardTitle>
        </CardHeader>
                  ...
                  <div className="flex justify-between items-center border-t border-white/5 pt-2.5 text-xs">
                    <div>
                      ...
                      <span className="text-[9px] text-white/40 font-mono block">
                        {tradingMode === 'REAL' ? formatSolToUsdAndHtg(b.capital).combinedLabel : `≈ ${formatUsdToHtg(b.capital)}`}
                      </span>
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 border-t border-white/5 pt-2.5 text-[10px]">
                    <div>
                      ...
                      <span className="text-[8px] text-white/40 font-mono block">
                        {tradingMode === 'REAL' ? formatSolToUsdAndHtg(b.netProfit || 0).combinedLabel : `≈ ${formatUsdToHtg(b.netProfit || 0)}`}
                      </span>
                    </div>
                    <div>
