...
export default function LeoCommanderWidget() {
  ...
  return (
    <>
      {/* Bouton Flottant Doré Responsive (Au-dessus de la barre de navigation sur Mobile) */}
      <button
      ...
      {/* Fenêtre Modale de Commandement Suprême (Optimisée 100% Mobile & Desktop) */}
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="w-[96vw] max-w-2xl max-h-[88vh] overflow-y-auto bg-[#0e0a1a]/95 border border-[#f0b90b]/40 text-white backdrop-blur-2xl rounded-2xl sm:rounded-3xl p-3.5 sm:p-6 shadow-[0_0_60px_rgba(240,185,11,0.25)]">
          ...
          {/* Baromètre Tactique & Métriques en Direct (Grille Responsive 2x2 Mobile / 4x1 Desktop) */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2 p-2 sm:p-2.5 rounded-xl sm:rounded-2xl bg-white/5 border border-white/10 text-center font-mono text-xs">
            ...
            <div className="p-1.5 sm:p-2 bg-black/40 rounded-lg sm:rounded-xl">
              ...
              <span className="text-[10px] sm:text-xs font-black text-white">
                ${fleetStatus?.totalAumUsd || '14,850'}
              </span>
            </div>
            <div className="p-1.5 sm:p-2 bg-black/40 rounded-lg sm:rounded-xl">
              <span className="text-[8px] sm:text-[9px] text-white/40 block">Coffre-Fort</span>
              <span className="text-[10px] sm:text-xs font-black text-[#c2ff0c]">
                ${fleetStatus?.vaultSecuredUsd || '1,485'}
              </span>
            </div>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={(val: any) => setActiveTab(val)} className="w-full">
            <TabsList className="grid grid-cols-3 bg-black/50 border border-white/10 rounded-xl p-0.5 sm:p-1">
              <TabsTrigger value="chat" className="text-[10px] sm:text-xs font-headline font-bold uppercase py-1 sm:py-1.5 data-[state=active]:bg-[#f0b90b] data-[state=active]:text-black">
                💬 Terminal
              </TabsTrigger>
              <TabsTrigger value="radar" className="text-[10px] sm:text-xs font-headline font-bold uppercase py-1 sm:py-1.5 data-[state=active]:bg-[#f0b90b] data-[state=active]:text-black">
                🌐 Radar Web
              </TabsTrigger>
              <TabsTrigger value="positions" className="text-[10px] sm:text-xs font-headline font-bold uppercase py-1 sm:py-1.5 data-[state=active]:bg-[#f0b90b] data-[state=active]:text-black">
                📊 Flotte
              </TabsTrigger>
            </TabsList>

            {/* ONGLET 1 : CHAT ET ORDRES RAPIDES */}
            <TabsContent value="chat" className="space-y-2.5 sm:space-y-3 mt-2 sm:mt-3">
              {/* 4 Boutons d'Ordres Rapides Responsive */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-8 sm:h-9 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border-rose-500/30 text-[9px] sm:text-[10px] font-headline font-bold rounded-lg sm:rounded-xl flex items-center justify-center gap-1 px-1.5"
                >
                  <Power className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Kill-Switch
                </Button>
              </div>

              {/* Terminal de Chat Responsive */}
              <div className="h-44 sm:h-52 overflow-y-auto space-y-2 sm:space-y-3 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-black/60 border border-white/10 text-xs">
                {chatLogs.map((log, i) => (
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSendCommand(); }} className="flex gap-1.5 sm:gap-2">
                <Input
                  ...
                  placeholder="Donnez un ordre à Léo..."
                  className="bg-black/60 border-white/15 text-white text-[11px] sm:text-xs rounded-xl focus:border-[#f0b90b] h-9 sm:h-10"
                />
                <Button type="submit" className="bg-[#f0b90b] hover:bg-yellow-400 text-black font-black font-headline text-xs px-3 sm:px-4 rounded-xl h-9 sm:h-10 shrink-0">
