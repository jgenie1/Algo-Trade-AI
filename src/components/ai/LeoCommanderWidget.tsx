...
export default function LeoCommanderWidget() {
  ...
  return (
    <>
      ...
      {/* Fenêtre Modale de Commandement Suprême */}
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-2xl bg-[#0e0a1a]/95 border border-[#f0b90b]/40 text-white backdrop-blur-2xl rounded-3xl p-6 shadow-[0_0_60px_rgba(240,185,11,0.25)]">
          <DialogHeader className="border-b border-white/10 pb-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <span className="text-lg font-black font-mono text-[#c2ff0c]">
          ...
          {/* Baromètre Tactique & Métriques en Direct */}
          <div className="grid grid-cols-4 gap-2 p-2.5 rounded-2xl bg-white/5 border border-white/10 text-center font-mono text-xs">
            <div className="p-2 bg-black/40 rounded-xl">
              <span className="text-[9px] text-white/40 block">Sentiment Web</span>
              <span className="text-xs font-black text-[#f0b90b]">
                {fleetStatus?.sentiment || 'GREED'} ({fleetStatus?.fearAndGreedIndex || 72}/100)
              </span>
            </div>
            <div className="p-2 bg-black/40 rounded-xl">
              <span className="text-[9px] text-white/40 block">Win-Rate 24h</span>
              <span className="text-xs font-black text-emerald-400">
                {fleetStatus?.winRate24h || 78.5}%
              </span>
            </div>
            <div className="p-2 bg-black/40 rounded-xl">
              <span className="text-[9px] text-white/40 block">AUM Global</span>
              <span className="text-xs font-black text-white">
                ${fleetStatus?.totalAumUsd || '14,850.00'}
              </span>
            </div>
            <div className="p-2 bg-black/40 rounded-xl">
              <span className="text-[9px] text-white/40 block">Coffre-Fort</span>
              <span className="text-xs font-black text-[#c2ff0c]">
                ${fleetStatus?.vaultSecuredUsd || '1,485.00'}
              </span>
            </div>
          </div>

          {/* Onglets de Navigation Léo */}
          <Tabs value={activeTab} onValueChange={(val: any) => setActiveTab(val)} className="w-full">
            <TabsList className="grid grid-cols-3 bg-black/50 border border-white/10 rounded-xl p-1">
              <TabsTrigger value="chat" className="text-xs font-headline font-bold uppercase data-[state=active]:bg-[#f0b90b] data-[state=active]:text-black">
                💬 Terminal Léo
              </TabsTrigger>
              <TabsTrigger value="radar" className="text-xs font-headline font-bold uppercase data-[state=active]:bg-[#f0b90b] data-[state=active]:text-black">
                🌐 Radar Web & Alpha
              </TabsTrigger>
              <TabsTrigger value="positions" className="text-xs font-headline font-bold uppercase data-[state=active]:bg-[#f0b90b] data-[state=active]:text-black">
                📊 Positions & Flotte
              </TabsTrigger>
            </TabsList>

            {/* ONGLET 1 : CHAT ET ORDRES RAPIDES */}
            <TabsContent value="chat" className="space-y-3 mt-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <Button
                  ...
                  variant="outline"
                  className="h-9 bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 border-blue-500/30 text-[10px] font-headline font-bold rounded-xl flex items-center justify-center gap-1"
                >
                  <Globe className="h-3.5 w-3.5" /> Scruter le Net
                </Button>
                <Button
                  onClick={() => handleSendCommand("Léo, lance une attaque avec le bot sniper sur la meilleure opportunité")}
                  variant="outline"
                  className="h-9 bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 border-purple-500/30 text-[10px] font-headline font-bold rounded-xl flex items-center justify-center gap-1"
                >
                  <Zap className="h-3.5 w-3.5 text-[#f0b90b]" /> Ordonner Trade
                </Button>
                <Button
                  onClick={() => handleSendCommand("Léo, sécurise tous les profits dans le coffre-fort")}
                  variant="outline"
                  className="h-9 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border-emerald-500/30 text-[10px] font-headline font-bold rounded-xl flex items-center justify-center gap-1"
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-9 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border-rose-500/30 text-[10px] font-headline font-bold rounded-xl flex items-center justify-center gap-1"
                >
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSendCommand(); }} className="flex gap-2">
                <Input
                  ...
                  placeholder="Donnez un ordre à Léo (ex: 'Scrute le net', 'Attaque avec le sniper', 'Sécurise les gains')..."
            ...
            {/* ONGLET 2 : RADAR WEB & OPPORTUNITÉS ALPHA */}
            <TabsContent value="radar" className="space-y-3 mt-3">
              <div className="flex justify-between items-center text-xs">
                <span className="text-white/60">Scrutage en direct (Pump.fun, CoinMarketCap, DexScreener) :</span>
                <Button onClick={refreshFleetData} variant="ghost" size="sm" className="h-7 text-[10px] text-[#f0b90b] gap-1">
                  <RefreshCw className={`h-3 w-3 ${isScanning ? 'animate-spin' : ''}`} /> Actualiser
                </Button>
              </div>

              <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                {(fleetStatus?.topOpportunities || []).map((opp, idx) => (
                  <div key={idx} className="p-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl flex items-center justify-between font-mono text-xs transition-colors">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-white text-sm">{opp.name}</span>
                        <Badge className="bg-[#f0b90b]/20 text-[#f0b90b] border border-[#f0b90b]/30 text-[9px]">
                          Alpha {opp.alphaScore}/100
                        </Badge>
                        <Badge className="bg-emerald-500/20 text-emerald-300 text-[9px]">
                          {opp.signal}
                        </Badge>
                      </div>
                      <p className="text-[10px] text-white/50 font-body mt-0.5">{opp.reasoning}</p>
                    </div>

                    <Button
                      onClick={() => handleSendCommand(`Léo, ordonne au ${opp.targetBot} d'acheter ${opp.name}`)}
                      className="bg-[#f0b90b] hover:bg-yellow-400 text-black font-headline font-bold text-[10px] h-8 rounded-lg px-3"
                    >
                      Ordonner Trade
                    </Button>
                  </div>
            ...
            <TabsContent value="positions" className="space-y-3 mt-3">
              <div className="text-xs text-white/60">
                Positions actives supervisées par Léo ({fleetStatus?.totalPositionsCount || 0}) :
              </div>

              <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                {(fleetStatus?.activePositionsSummary || []).length === 0 ? (
                  <div className="p-6 text-center text-white/40 text-xs font-mono bg-black/40 rounded-xl border border-white/5">
                    Aucune position ouverte. Les 5 bots scrutent le marché.
                  </div>
                ) : (
                  (fleetStatus?.activePositionsSummary || []).map((p, idx) => (
                    <div key={idx} className="p-2.5 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between font-mono text-xs">
                      <div>
                        <span className="font-bold text-white">{p.pair}</span>
                        <span className="text-[10px] text-white/40 ml-2">({p.botName})</span>
                      </div>
                      <div className="text-right">
                        <span className={`font-bold ${p.pnlUsd >= 0 ? 'text-[#c2ff0c]' : 'text-rose-400'}`}>
                          {p.pnlUsd >= 0 ? '+' : ''}${p.pnlUsd.toFixed(2)}
                        </span>
                        <span className="text-[10px] text-white/50 ml-1.5">({p.pnlPct.toFixed(2)}%)</span>
                      </div>
                    </div>
                  ))
                )}
